...
class BreedSelectorTableViewController: UITableViewController, UISearchBarDelegate, UISearchResultsUpdating {
        ...
        if on {
            ...
            if let max = self.max {
                if max == 1 && selection.count >= 1 {
                    let indexPath = IndexPath(row: selection[0], section: 0)
    ...
    func getSelectionIndex(fromBreedId id: Int) -> Int? {
        let index = App.instance.breeds.index { $0.id == id }
        if let index = index {
            return selection.index(of: index)
        }
        return nil
    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let idx = getSelectionIndex(fromBreedId: breed.id) {
            selection.remove(at: idx)
            tableView.deselectRow(at: indexPath, animated: true)
            cell?.accessoryType = .none
        } else {
            
            selection.append(App.instance.breeds.index { $0.id == breed.id }!)
            cell?.accessoryType = .checkmark
