...
class App {
    ...
    func requestUserBreedsAndAnimals() -> Promise<Void> {
        ...
        let user = UserModel.getProfilUser().then { json -> Void in
            let animals = self.userModel?.animals
            self.userModel = json
            self.userModel.animals = animals
        }
        let breeds = Api.instance.get("/breed").then { JSON in
            self.breeds = JSON["breeds"].arrayValue.map { BreedModel(fromJSON: $0) }
        }
        
        let animals = Api.instance.get("/animals/").then { JSON -> Void in
            let animalsJSON = JSON["myanimal"].arrayValue
            if self.userModel == nil {
                self.userModel = UserModel()
            }
            self.userModel.animals = animalsJSON.map { AnimalModel(fromJSON: $0) }
        }
        
        return when(fulfilled: [user, breeds, animals])
    }
